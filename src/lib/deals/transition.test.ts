...
const plan = (s: DealSignal, current: CurrentDeal | null = null) =>
  planTransition({ signal: s, current, stages: STAGES, fallbackTitle: 'Fulano' })

describe('planTransition — creating a card', () => {
  it('creates in Lead Qualificado with the model as the title', () => {
    const p = plan(signal({ outcome: 'qualified', model: 'iPhone 15 Pro Max 256GB' }))
    expect(p).toEqual({
      action: 'create',
      stageId: 'stage-qual',
  ...
  it('creates straight into Negociação with the quoted value', () => {
    const p = plan(signal({ outcome: 'negotiating', model: 'iPhone 14', price: 2700 }))
    expect(p).toEqual({
      action: 'create',
      stageId: 'stage-nego',
      title: 'iPhone 14',
...
describe('planTransition — invariants', () => {
  ...
  it('treats won as terminal — later chatter cannot un-win it', () => {
    for (const outcome of ['qualified', 'negotiating', 'lost', 'won'] as const) {
      const p = plan(
  ...
  it('does not re-mark an already-lost deal as lost', () => {
    const p = plan(signal({ outcome: 'lost' }), deal({ stagePosition: 2, status: 'lost' }))
    expect(p).toMatchObject({ action: 'none' })
  })
...
describe('matchStageSlots', () => {
  ...
  it('returns null when a stage is missing rather than guessing', () => {
    expect(matchStageSlots(board.filter((s) => s.name !== 'Negociação'))).toBeNull()
