...
// Link generation shares the same canonical-domain resolver as the
// public WhatsApp pairing flow, so neither feature leaks a stale host.
    ...
    return NextResponse.json(
      {
        ...
        token,
        url: inviteUrl(token, publicBaseUrl(request)),
