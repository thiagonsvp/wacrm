...
interface ContactRow {
  ...
  contact_tags?: { tags?: { name?: string | null } | { name?: string | null }[] | null }[]
}
 ...
 */
function bestDeal(deals: DealRow[], positionOf: (id: string | null) => number): DealRow {
  const rank = (d: DealRow) => (d.status === 'won' ? 3 : d.status === 'lost' ? 1 : 2)
  return deals.reduce((best, d) => {
    ...
    return (d.updated_at ?? d.created_at ?? '') > (best.updated_at ?? best.created_at ?? '')
      ? d
      : best
  })
}
 ...
 */
function bucketOf(deal: DealRow, stages: PipelineStageMap, position: number): FunnelBucket {
  if (deal.status === 'won') return 'won'
  if (deal.status === 'lost') return 'lost'
  if (stages.disqualified && deal.stage_id === stages.disqualified.id) return 'disqualified'
  if (position >= stages.negotiating.position) return 'negotiating'
  if (position >= stages.qualified.position) return 'qualified'
  return 'early'
}

function applyBucket(counts: FunnelCounts, bucket: FunnelBucket, value: number): void {
  counts.leads += 1
  switch (bucket) {
    case 'none':
      counts.noDeal += 1
...
export async function GET(request: Request) {
  try {
    ...
    if (configRes.error) {
      console.warn('[performance/funnel] ai_configs unavailable:', configRes.error.message)
    }
    const configured = {
      qualified: (configRes.data?.deal_stage_qualified_id as string | null) ?? null,
      negotiating: (configRes.data?.deal_stage_negotiating_id as string | null) ?? null,
      closed: (configRes.data?.deal_stage_closed_id as string | null) ?? null,
    }

    let stageMap: PipelineStageMap | null = mapConfiguredStages(configured, allStages)
    if (!stageMap) {
      for (const pipelineId of pipelineIds) {
        const slots = matchStageSlots(allStages.filter((s) => s.pipeline_id === pipelineId))
        if (slots) {
    ...
    const isExcluded = (c: ContactRow) =>
      (c.contact_tags ?? []).some((join) => {
        const tags = Array.isArray(join.tags) ? join.tags : join.tags ? [join.tags] : []
        return tags.some(
          (t) => t?.name && excludedTags.some((excluded) => sameTag(t.name as string, excluded)),
        )
      })
    ...
    for (const { row, day } of leads) {
      ...
      const origin = row.acquisition_source ?? (row.acquisition_source_id ? 'Meta Ads' : 'Não informado')
    ...
    const payload: FunnelPayload = {
      currency: (account.data?.default_currency as string | undefined) ?? 'BRL',
      stageNames: stageMap
        ? {
            qualified: stageById.get(stageMap.qualified.id)?.name ?? 'Qualificado',
            negotiating: stageById.get(stageMap.negotiating.id)?.name ?? 'Negociação',
