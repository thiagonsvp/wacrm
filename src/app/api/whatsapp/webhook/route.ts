...
async function processMessage(
  ...
  const protocolMatch = message.referral
    ? null
    : await resolveGoogleAdsProtocol(
        supabaseAdmin(),
        accountId,
        message.text?.body
      );
  ...
  // Reactions short-circuit here — they aren't messages, and Evolution
  // has no equivalent event, so this stays Meta-only in the route
  // rather than in the shared inbound module. Done before
  // parseMessageContent so the media-URL fetch is skipped. Note this
  // means a reaction that opens a brand-new conversation still needs
  // conversation.created dispatched — handled by persistInboundMessage
  // for the message path; reactions never call that, so replicate the
  // event here for parity with the old inline behavior.
  ...
  if (message.type === 'audio' && !contentText && message.audio?.id) {
    try {
      ...
      console.error(
        '[webhook] audio download for transcription failed:',
        error
      );
    }
  }
...
// src/lib/whatsapp/inbound.ts, shared with the Evolution webhook.
