...
export function AiConfig() {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      setHasGlobalKey(Boolean(data.has_global_key));
  ...
  const handleSave = async () => {
    if (!hasGlobalKey) {
      toast.error('Configure a chave e o modelo OpenAI nas configurações do administrador.');
      return;
    }
    setSaving(true);
    try {
    ...
    <div>
      ...
      <div className="space-y-6">
        <p className="rounded-md border border-border bg-muted/40 px-3 py-2 text-sm text-muted-foreground">
          A chave e o modelo OpenAI são gerenciados nas configurações do administrador e valem para todas as empresas.
        </p>
