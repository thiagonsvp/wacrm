...
export function DealForm({
  ...
  async function handleStatusChange(status: DealStatus) {
    ...
    if (status === 'won' && !expectedCloseDate) {
      toast.error(
        'Informe a data de fechamento antes de marcar a venda como ganha.'
      );
      return;
    }
            ...
            <div className="grid grid-cols-[1fr_110px] gap-3">
              <div className="grid gap-2">
                ...
                <div className="relative">
                  <Banknote className="text-muted-foreground absolute top-1/2 left-2 h-3.5 w-3.5 -translate-y-1/2" />
                  <Input
                    type="number"
                    value={value}
              ...
              <div className="grid gap-2">
                ...
                <select
                  value={currency}
                  disabled
                  className="border-border bg-muted text-foreground h-9 w-full rounded-lg border px-2.5 text-sm opacity-100"
                >
