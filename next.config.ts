...
const nextConfig: NextConfig = {
    ...
    return [
      {
        ...
        has: [
          {
            type: 'host',
            value: 'wacrm-eight-pi.vercel.app',
          },
        ],
        destination: 'https://crm.natividadedigital.com.br/:path*',
        permanent: true,
      },
    ];
  },
  ...
  async headers() {
    return [
      ...
      {
        source: '/:path((?!_next/static|_next/image|api|connect-whatsapp).*)',
        headers: [
          {
            key: 'Cache-Control',
            value:
      ...
      {
        // The bearer token lives in this path. Never let a CDN retain it
        // and never send it as a referrer, even if the page changes later.
        source: '/connect-whatsapp/:path*',
        headers: [
